...
var $author$project$Page$Home$Model = F2(
	function (title, latestsPosts) {
		return {latestsPosts: latestsPosts, title: title};
	});
...
var $author$project$Page$Home$latestPosts = _List_fromArray(
	[
		A4($author$project$Page$Home$LatestPost, 'Espresso is missing views', 'Espresso may be lacking of information...', 'src/espresso.png', 1),
		A4($author$project$Page$Home$LatestPost, 'Welcome', 'This is a testing post, nothing much to look here...', 'src/lisa.jpeg', 0)
	]);
...
var $author$project$Page$Home$init = F2(
	function (title, latestsPosts) {
		return A2($author$project$Page$Home$Model, title, latestsPosts);
	});
...
var $author$project$Main$PostsMsg = function (a) {
	return {$: 'PostsMsg', a: a};
};
	...
	function (model, _v0) {
		...
		return _Utils_Tuple2(
			_Utils_update(
				model,
				{
					page: $author$project$Main$Posts(post)
				}),
			A2($elm$core$Platform$Cmd$map, $author$project$Main$PostsMsg, cmds));
	});
			...
			_List_fromArray(
				[
					...
					A2(
						...
						model,
						_Utils_Tuple2(
							A2($author$project$Page$Home$init, 'Home', $author$project$Page$Home$latestPosts),
		...
		return A2(
			...
			url,
			{
				...
				page: $author$project$Main$Home(
					A2($author$project$Page$Home$Model, 'Home', $author$project$Page$Home$latestPosts))
			});
	});
		...
		switch (message.$) {
			...
			case 'LinkClicked':
				...
				if (urlRequest.$ === 'Internal') {
					...
					var fafa = A2($elm$core$Debug$log, 'Url:', url);
					return _Utils_Tuple2(
						model,
						A2(
...
var $author$project$Components$Navbar$ulStyle = _List_fromArray(
	[
...
var $author$project$Components$Navbar$navbarTab = F2(
	function (attributes, tabInfo) {
		var image = A2(
			$elm$html$Html$div,
			_List_fromArray(
				[
			...
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$Attributes$src('src/assets/' + tabInfo.imageResource),
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									$elm$html$Html$text(tabInfo.title)
								]))
						]))
				]));
		var _v0 = tabInfo.onPressed;
		if (_v0.$ === 'Just') {
			var m = _v0.a;
			return A2(
				$elm$html$Html$li,
...
var $author$project$Components$Navbar$navbar = function (tabsInformation) {
	return A2(
		$elm$html$Html$nav,
		_List_Nil,
		_List_fromArray(
			[
				$author$project$Components$Navbar$navbarContainer(
				A3(
					$elm$core$List$foldr,
					F2(
						function (tabInfo, navigationTabs) {
							return A2(
								$elm$core$List$cons,
								A2($author$project$Components$Navbar$navbarTab, _List_Nil, tabInfo),
								navigationTabs);
						}),
					_List_Nil,
					tabsInformation))
			...
			return A2(
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								$elm$html$Html$text(lp.shortText)
							])),
						A2(
						$elm$html$Html$img,
						_List_fromArray(
							[
								$elm$html$Html$Attributes$src(lp.thumbnailResource),
...
var $author$project$Main$view = function (model) {
	return $author$project$Main$wrapperFor(model.page);
};
